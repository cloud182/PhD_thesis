...
This behavior can be explained thanks to the Unified Model of AGN \citep{Antonucci93,Urry95}.
...
Moreover, ionization cones seem to be often aligned with the radio jets of the AGN \citep{Unger87,Nagar99,Schmitt03,Schmitt03b} and in some case it has been proved that shocks can be important in the ionization of the ENLR gas \citep{Congiu17b,Rodriguez17}.
...
As mentioned in the previous chapter of this work, NLS1s are a peculiar class of Type 1 AGN, classified for the first time by \citet{Osterbrock85}.
...
The analysis of new spectra of Mrk\,783 \citep{Congiu17c} revealed that the strongest optical emission lines can be traced up to $\sim 30^{\prime\prime}$ from the nucleus, which corresponds to a projected size of $\sim 38\,\si{kpc}$, making this ENLR one of the most extended discovered so far at $z<0.1$.

In this chapter, I present a detailed analysis of several spectra and images of Mrk\,783 to characterize the properties of the emission, and of the host galaxy in general, and to study the relation between the ENLR and the extended radio emission.
...
In this work, I adopt the following cosmological parameters: $H_0 = 70\,\si{\kms.Mpc^{-1}}$, $\Omega_{m,0} = 0.27$ and $\Omega_{\Lambda,0} = 0.73$ \citep{Komatsu11}.

\section{Mrk 783}
\label{sec:pap3_mrk783}

Mrk 783 ($\alpha$ = $13^{\rm h}$ $02^{\rm m}$ $58.8^{\rm s}$ $\delta$=$+16^{\circ}$ $24^{\prime}$ $27^{\prime\prime}$) is a NLS1 galaxy firstly classified by \citet{Osterbrock85} at $z = 0.0672$ \citep{Hewitt91}.
...
Mrk 783 appears as a point-source, without any extended emission, and with a brightness temperature of $7.7\times10^7\,\si{K}$, a sign of the non-thermal origin of this emission.

\section{Observations}
\label{sec:obs}

In order to investigate the presence of the extended optical emission associated with the KSR in this galaxy, I proposed new spectroscopic and imaging observations.
The main purpose of the former was to detect and characterize the property of the putative ENLR and of the environment in which the galaxy resides.
The latter were proposed to get a better insight into the morphological properties of the source and to identify eventual companion galaxies.
...
\begin{table}
    ...
    \hline
    \hline
    Date& PA &Slit width& Exp.& Time &  $\Delta\lambda$ & RMS &Standard&Instrument\\
    & & & (arcsec) & (s) & ($\si{\angstrom}$)& ($\si{\angstrom}$) & &\\   
    \hline
    2017-02-22 & $\ang{131}$ & $1.0$& $3$ & $3600$ & $8.2$ & $0.87$ & LTT 4816&LDSS-3\\
    2017-06-09 & $\ang{90}$  & $1.0$&$3$ & $3600$ & $7.9$ & $1.50$ & EG 274&LDSS-3\\
               & $\ang{131}$ & $1.0$&$3$ & $3600$ & $7.9$ & $0.52$ & EG 274&LDSS-3\\
    2018-06-11 & $\ang{333.5}$ & $1.0$&$3$ & $2700$ &$10.04$&-&-& DOLORES\\
    \hline
    \end{tabular}
    \normalsize
    \begin{tablenotes}
    \item From left to right: (1) date of the observation, (2) position angle, (3) width of the slit in arcseconds, (4)number of exposures per pointing, (5) total exposure time, (6) resolution of the spectrum measured from the calibration lamp spectrum, (7) error on the wavelength calibration measured on the sky lines, (8) standard star used for flux calibration, (10) instrument used.
    \end{tablenotes}
    \end{threeparttable}
\end{table}
...
The spectra at PA $=\ang{90}$ and $\ang{131}$ were observed with LDSS-3@Magellan, while the spectrum at PA $=\ang{333.5}$ was acquired with DOLORES@TNG.
Only one of the two PA $=\ang{131}$ spectra is shown in the figure, since the only difference is that in the second spectrum the position of the nucleus on the slit has been shifted, in order to avoid the slit support covering part of the extended emission.}
...
All the observations have been performed in long slit mode, with the $1^{\prime\prime}\times4^{\prime}$ ``center'' slit and the VPH-All.
...
Before the subtraction, I used the calibration lamp spectra and some sky emission lines to estimate the resolution of the science data and the reliability of the wavelength calibration.
...
I tried to improve this calibration using the sky lines as a reference for the calibration process.
...
\begin{table}
    ...
    \hline
    \hline
    Instrument& Filter & Exp.& Exp. time & Seeing & SB limit & Date \\
    & & & (s) & ($\si{arcsec}$)& ($\si{mag}$)&\\   
    \hline
...
The g-band image of Mrk 783 has a measured $3\sigma$ surface brightness limit of $\sim 24.7\,\si{mag.arcsec^{-2}}$.
...
The galaxy has been observed for a total exposure time of $2\,\si{hr}$ divided into $12$ single $600\,\si{s}$ exposures. 
...
The seeing during the observations was highly variable, therefore, before the combination, they were all convolved with a Gaussian kernel in order to obtain a uniform seeing corresponding to the worst seeing value.
...
The position of the slit has been chosen in order to be aligned with the axis of the radio emission discovered in \citet{Congiu17}, since its purpose was to verify the presence of optical radio emission aligned with the radio structure.

This spectrum revealed, indeed, extended emission in all the strongest emission lines ([\ion{O}{III}]$\lambda5007$ in particular, Fig.\,\ref{fig:three_spectra}, panel a) on both sides of the AGN.
The [\ion{O}{III}]$\lambda5007$ line can be traced up to a distance of $\sim 25^{\prime\prime}$ from the nucleus of the galaxy, which corresponds to a projected distance of $\sim 32\,\si{kpc}$.
In this spectrum, the emission stops sharply, because of the presence of an interruption of the slit.
...
The $\Hb$ region of the new spectrum is shown in Fig.\,\ref{fig:three_spectra} (panel b).
As expected, the emission is slightly more extended and it reaches $\sim 30^{\prime\prime}$ ($\sim 38\,\si{kpc}$) from the nucleus.
...
One possibility for this behavior might be that the most extended emission is produced by a nearby source.
...
The spectrum in the panel c of Fig.\,\ref{fig:three_spectra} shows the $\Hb$ region of the last LDSS-3 spectrum, acquired at PA $=\ang{90}$.
...
An interesting property that can be observed in panel c of Fig.\,\ref{fig:three_spectra} is that there seems to be a second continuum $ 1.5\,\si{arcsec}$ west with respect to the AGN, that can be better appreciated in Fig.\,\ref{fig:flux_distr}.
These last figures show the distribution of light along the slit in a single dispersion element.
...
\caption[]{\textbf{Left:} SDSS \emph{g}-band image of Mrk 783. \textbf{Center:} SDSS \emph{r}-band image of Mrk 783. \textbf{Center:} SDSS \emph{i}-band image of Mrk 783. Each image is in units of specific intensity ($\si{counts.cm^{-2}.s^{-1}}$). The contours are defined as $3(2^n)\sigma_{sky}$, where $\sigma_{sky}$ is the RMS of the background and $n\ge 0$. The red cross marks the coordinates of the center of the galaxy as reported by the NED. }
\label{fig:contours}
\end{figure}

The galaxy has been classified as a lenticular galaxy by \citet{Petrosian07}.
...
The images show that the faint structure observed in the RGB image is real, even though is barely over the $3\sigma$ detection limit.
...
Fig.\,\ref{fig:contours} also shows that there seems to be a light excess $\sim 1.5\,\si{arcsec}$ west of the AGN, in the position corresponding to the second continuum observed in Fig.\,\ref{fig:three_spectra} and \ref{fig:flux_distr}.
This excess becomes more prominent moving towards redder wavelengths, i.e. the shape of the contours deviate from an ellipse more in the \emph{r} and \emph{i} images than in the \emph{g} image.
...
The images have not been corrected for Milky Way extinction, but according to the calibration of \citet{Schlafly11} it is of the order of $0.09\,\si{mag}$ in the \emph{g}-band and $0.07\,\si{mag}$ in \emph{r}-band, therefore it will not particularly influence these results.

\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{images/paper3/mrk783color.pdf} 
\caption[]{$g-r$ image of Mrk 783. The black cross marks the NED coordinates of the object. It is possible to see how the galaxy is sharply divided into two regions. On the left, the AGN has a negative color while the second point-like structure has a positive color. The difference in color between the two sides is $\sim 2\,\si{mag}$.} 
\label{fig:mrk_gr}
\end{figure} 
...
These events are rare but not impossible since such scenario has been predicted by numerical simulations and some candidates have already been discovered \citep[for a review see][]{Komossa12}.
...
FIRST and NVSS data, together with \citet{Congiu17} measurements have been used to constrain the synchrotron emission.
...
The kinematics might also reveal if one of the two structure has been accreted in a merging event or if they both belong to the galaxy.
In the first case the kinematics of the accreted structure should be disturbed with respect to the kinematics of the host galaxy itself, while in the second case there should not be any difference.
...
The difference between the two wavelengths of the two line is $\sim 0.7\,\si{\angstrom}$ ($\sim 50\,\si{\kms}$).
This confirms that the vicinity of the two objects is not just a projection effect.
...
Actually, the two objects are close enough that they might be already interacting, even though the undisturbed morphology of J1302+1625 suggests that the interaction is still in the early phases.
...
An accurate measurement of the emission line flux, especially for the Balmer lines, usually requires a subtraction of the underlying stellar continuum, since stellar absorption might significantly contribute to the line flux.
...
Since the SNR of the continuum is low in most of the bin extracted from the LDSS-3 spectra uncontaminated by the AGN (SNR$<10$), I decided to not perform the stellar continuum subtraction.
...
In the very nucleus of the galaxy, I also had to fit the [\ion{S}{II}] doublet  simultaneously with the $\Ha$ group, because the broad components of $\Ha$ were so large to be partially blended with the doublet.
...
\caption[]{One-dimensional spectrum of the nucleus of Mrk 783 extracted from all the three LDSS-3 spectra.} 
...
For these reasons, the spectra have not been combined to produce a single spectrum of the nucleus but they will be examined separately in the following sections.
Fig.\,\ref{fig:all_spectra} shows the AGN spectrum extracted from each one of the LDSS-3 spectra.
There are some small differences between the three spectra, probably due to the different sky condition or by a small error in positioning the AGN inside the slit.
However, to check the classification of the AGN as a NLS1 I will only consider one of the spectra.
Since the spectra are very similar and there is not any advantage in choosing one over the others, in this section I will always refer to the February spectrum at PA $=\ang{131}$.
...
\caption[]{Diagnostic diagrams for the LDSS-3 spectra. From top to bottom February PA $=\ang{131}$, June PA $=\ang{131}$ and June PA $=\ang{90}$. For each spectrum, the left panel is the  $\log [\rm \ion{O}{III}]\lambda5007/\Hb$ vs $\log [\rm \ion{N}{II}]\lambda6584/\Ha$ diagram, the central panel is the $\log [\rm \ion{O}{III}]\lambda5007/\Hb$ vs $\log [\rm \ion{O}{I}]\lambda6300/\Ha$ diagram while the right panel is the $\log [\rm \ion{O}{III}]\lambda5007/\Hb$ vs $\log [\rm \ion{S}{II}]\lambda\lambda6717,6731/\Ha$. The color bar shows, for each point, the distance from the nucleus of the corresponding spectrum.
In each panel, the solid curve is the \citet{Kewley01} relation dividing AGN from star-forming regions, the dashed curve is the \citet{Kauffmann03} line for pure star formation while the dashed lines are the \citet{Kewley06} relations dividing Seyfert galaxies from LINERs.
...
Therefore, it is possible that the subtraction of the stellar continuum might move some other points in the mixed region or shift some of the points already in this region into the \ion{H}{II} one.
...
However, it is clear from the diagnostic diagrams in Fig.\,\ref{fig:pap3_diagnostic} that low [\ion{n}{II}]$\lambda6584/\Ha$ values do not mean that the region cannot be ionized by the AGN, but only that this ratio is not sufficient to realistically estimate the ionizing source of the gas.
...
\caption[]{\textbf{Top:} $\log [\rm \ion{O}{III}]\lambda5007/\Hb$ vs distance from the nucleus plot for February PA $=\ang{131}$ spectrum. The dashed line represents the $0.6$ limit.
\textbf{Middle:} same plot as the previous panel for June PA $=\ang{131}$ spectrum.
\textbf{Bottom:} $\log [\rm \ion{n}{II}]\lambda6584/\Ha$ plot for June PA $=\ang{90}$ spectrum. In this plot, the dashed line represents the $-0.5$ limit for mixed ionization and the dotted line is the $-0.1$ limit for AGN ionization. As in the diagnostic diagrams, the error bars represent $1\sigma$ error on the line ratios. Absent bars means that they are smaller than the size of the point.}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{images/paper3/mrk783_OR.jpg}\\  
\caption[]{VLA 5 GHz contours from the natural weighted image of \citet{Congiu17} superimposed to the Asiago \emph{g} band image of Mrk 783. The position of the slit during LDSS-3 observations at PA $=\ang{131}$ is shown. }
\label{fig:OR_map}
\end{figure}

The data analyzed in this work have been acquired as a follow up of the source after the discovery of a peculiar kiloparsec-scale radio emission region (KSR) \citep{Congiu17}.
...
Fig\,\ref{fig:OR_map} shows how the bulk of the radio emission is located inside the nucleus of the galaxy, even though the jet might have been able to dig in the densest part of the ISM.
Fig\,\ref{fig:OR_map} also shows how the emission on the north-west side of the nucleus is not visible in the natural weighted image, in agreement with the fact that the optical emission in that region is less extended.
...
Since the morphology of the ENLR is not known, it is not possible to definitively confirm that the axes of the two extended regions are aligned.
...
Both UGC 7342 and NGC 5972 show a disturbed morphology and UGC 7342 is an ongoing merger.
Since the gravitational interaction produced during a merger influences the gas distribution, it is possible that it can create the conditions needed to produced larger ENLR than in stable systems.
...
Merging events are now widely accepted as one of the few mechanisms that are able to bring gas close to the supermassive black hole of a galaxy, supplying it of the gas needed for the accretion \citep{Sanders88,Hong15}.
...
Assuming that the more extended part of the spectra at PA $=\ang{131}$ and PA $=\ang{90}$ belongs to the same cone, it is possible to estimate a minimum opening angle of $\sim \ang{139}$, which is quite large but not outside the possible values found by \citet{He18}.
On the other hand, if the more extended regions are part of opposite cones, the minimum opening angle is reduced to $\ang{41}$.
...
From the images it is not clear the nature of this emitting region, it can be both a single cloud or part of a faint tidal tail.
...
Understanding the relationship between the ENLR and the radio emission, the merging events and all the other properties of this galaxy will be of the uttermost importance to shed light on the feedback processes between the AGN and its host. 

A wider study to detect and characterize other similar objects will be fundamental to understand if Mrk 783 is only a ``lucky case'' and what is its position in the class of NLS1 and AGN in general. 
