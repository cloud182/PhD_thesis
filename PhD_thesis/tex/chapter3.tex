...
He analyzed medium resolution data of the archetypal Seyfert 2 galaxy NGC 1068, dividing the emission lines in velocity bins and analyzing the properties of the gas in each bin in order to study their variation as a function of velocity.
...
MagE is able to cover simultaneously the complete visible spectrum ($3100$ -- $10000\,\si{\angstrom}$) with a maximum resolution $\rm R=8000$ or $\sim 35\,\si{km.s^{-1}}$\citep{Marshall08}.
...
While IC 5063 spectrum has been analyzed for the first time in this work, the NGC 7212 data have been already published in \citet{Cracco2011}.


\begin{table*}
\caption[]{Principal properties of the observed galaxies, taken from the NED.}
\label{tab:sample}
\centering
\footnotesize
...
\begin{tablenotes}
\item $^1$\cite{Munoz07}
\item From left to right: 1) name of the object, 2) total exposure time, 3) Position angle of the slit during the observations, 4) slit width, 5) dispersion of the spectra, 8) seeing of the observations, as measured from the close standard star observed during the night , 9) date of observations
\end{tablenotes}
\end{threeparttable}
\normalsize
\end{table*}
...
The final SNR ratio on the continuum at the rest wavelength of $5500\,\si{\angstrom}$ varies from $\rm SNR \sim 25$ in the CN region of IC 5063 to $\rm SNR \sim 18$ in the N2 region.
...
Also in this galaxy the measurements have been performed at a rest wavelength of $5500\,\si{\angstrom}$.
...
The errorbar are smaller than the size of the points. The black curves in (1), (2), (4) divide power-law ionized regions (top) and H II regions (bottom). The red lines divide Seyfert-like regions (left) and LINER-like regions (right) \citep{Kewley06}. The black lines in (3) divide H II regions (bottom), power-law ionized region (left) and shock ionized regions (right) \citep{Baldwin81}.  }
...
This behavior might be explained as a consequence of shocks, which affect the spectrum lowering the ionization degree of the gas (low [\ion{O}{III}]$\lambda5007/\Hb$ ratio) and increasing the amount of ions in a low ionization state (high [\ion{O}{I}]$\lambda6300/\Ha$, [\ion{N}{II}]$\lambda6584/\Ha$ and [\ion{S}{II}]$\lambda\lambda6716,6731/\Ha$).
...
The spectra were all shifted to rest frame using stellar kinematics, but, outside the nucleus, the main peak of each line is often shifted toward longer or shorter wavelengths of $\sim100\,\si{\kms}$, probably caused by bulk motions of gas that it is not visible in the stellar component of the galaxies.
...
IC 5063 shows blushifted lines in the northern region of the spectrum and redshifted lines in the southern part. 
...
Neither of the peaks is positioned at $V=0\si{\kms}$, meaning that the kinematics of the gas is different from the one of the stellar component, as stated in the introduction of Sec.\,\ref{sec:lineprof}.
...
The maximum of the line is located at $V=0\,\si{\kms}$, even though the flat top of both [O III]$\lambda5007$ and $\Hb$ makes the extimate relatively unprecise. 
...
In this region the main peak is furthed shifted to the red, up to a velocity $V \sim 160\,\si{\kms}$.
...
Here, all the lines show a secondary peak which is $10$ to $20$ per cent stronger with respect to the same feature visible in $[\rm\ion{O}{III}]\lambda5007$  (Fig.\,\ref{fig:s1l1_I}), with the exception of $[\rm\ion{O}{I}]\lambda6300$ which is very similar to the $[\rm\ion{O}{III}]$ line.
...
Both components are visible also in both the central regions (CN and CS, Fig.\,\ref{fig:cnl1_N}--\ref{fig:csl1_N}).
...
In the CS region the dominant component become the blue one, while the red one is the fainter one.
...
On the other hand, $[\rm\ion{O}{III}]\lambda5007$ shows a peak at $V\sim -50\,\si{\kms}$ and a relatively strong bump ($>80$ per cent of the peak intensity) in the velocity where the other lines have the peak.
