...
ic50sed = np.genfromtxt('ic50sed.dat', unpack = True)
...
fort113_3=10**(fort113_ic3[1]-19)
galaxy = np.log10(aa+fort113_3) 

#### summing free-free

x_f = fort12g[0]
free1 = 10**(fort12g[1]-10.)
...
x_d = fort12d[0]
dust1 = 10**(fort12d[1]-12.1)
dust2 = 10**(fort12_100d[1]-10.6)
dust3 = 10**(fort12_600d[1]-15)
...
new_sync = 10**(sync_i(x_new_s)-9.3)
x_new_a = np.arange(15.32,19.97,0.08)
new_agn = 10**(agn_i(x_new_a)-12.8)
...
fig, ax = plt.subplots(1,1)grey
ax.axis([7,20,-16.5,-8.])
ax.plot(x_def,total_sed, c='k', label = 'Best fit')
ax.plot(fort18[0],fort18[1]-12.8, c= 'c', ls = '--',label='AGN')
...
ax.plot(sync[0],sync[1]-9.3, c='b', ls= '--', label='Syncrothron')
ax.errorbar(np.log10(ic50sed[0]),np.log10(ic50sed[1])+3,yerr=0, ls = '', marker = 'o', markerfacecolor = 'none', markeredgecolor='k', label = 'IC 5063')
ax.legend(loc='best')
ax.set_xlabel('log($\\nu$) (Hz)')
ax.set_ylabel('log($\\nu$F$_{\\nu}$) (erg cm$^{-2}$ s$^{-1}$)')
ax.set_title('IC 5063')
plt.savefig('sedic50n.pdf')
plt.close()
